...
public class MessageRecordRedis {
    ...
    public void getAndInsertMessageFromUrl(UrlRecord value,Boolean first,String title){
        ...
        if(first){
            List<RssPojo> rssFromUrl = rssReaderUtil.getRssFromUrl(url,title);
            ...
            if(levelNow==0){
                List<RssPojo> rssFromUrl = rssReaderUtil.getRssFromUrl(url,null);
                redisTemplateLevel.opsForValue().set(tableName,level);
                System.out.println(tableName+"级别重置"+level);
                if(rssFromUrl.size()>0) insert(rssFromUrl,url);
                else {
                    System.out.println(tableName+"无获取rss值");
                }
            }else{
                redisTemplateLevel.opsForValue().decrement(tableName);
                System.out.println(tableName+"级别： "+redisTemplateLevel.opsForValue().get(tableName));
            }
        }
    }
}
